...
impl RdmaQueuePair {
    ...
    pub fn connect(&mut self, connection_info: &RdmaQpInfo) -> Result<(), anyhow::Error> {
        ...
        if self.is_efa() {
            return self.connect_via_c(connection_info);
        }
    ...
    /// Connects via the EFA-specific C function.
    fn connect_via_c(&mut self, connection_info: &RdmaQpInfo) -> Result<(), anyhow::Error> {
        let qp = self.qp as *mut rdmaxcel_sys::rdmaxcel_qp;

        let gid_ptr = connection_info.gid.as_ref().map_or(
            std::ptr::null(),
            |g| {
    ...
    fn post_op(
        ...
        if self.is_efa() {
            return self.post_op_via_c(laddr, lkey, length, wr_id, signaled, op_type, raddr, rkey);
        }
    ...
    /// Posts via the unified C function (used for EFA).
    fn post_op_via_c(
        &mut self,
        laddr: usize,
        lkey: u32,
        ...
        unsafe {
            ...
            if ret != 0 {
                let msg = std::ffi::CStr::from_ptr(rdmaxcel_sys::rdmaxcel_error_string(ret))
                    .to_str()
                    .unwrap_or("unknown");
                return Err(anyhow::anyhow!("Failed to post {:?} request: {}", op_type, msg));
            }
            tracing::debug!(
                "completed sending {:?} request (lkey: {}, addr: 0x{:x}, length {}) to (raddr 0x{:x}, rkey {})",
                op_type,

...
async fn create_rdma_buffer(
    ...
    if efa_supported() {
        ...
        let buffer = owner_ref
            ...
            .await?;
        Ok(PyRdmaBuffer { inner: BufferInner::Ibverbs { buffer, owner_ref } })
    }
}
...
impl PyRdmaBuffer {
    ...
    fn size(&self) -> usize {
        match &self.inner {
            BufferInner::Ibverbs { buffer, .. } => buffer.size,
            BufferInner::Efa { buffer, .. } => buffer.size(),
        }
    }
